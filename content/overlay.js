...
;(() => {
	...
	try {
		...
		if (!article || !article.content) {
			container.innerHTML =
				`<div class="reader-empty">No readable article found.</div>`
		} else {
		...
		container.innerHTML = `<div class="reader-empty">Reader error: ${
			escapeHtml(String(e,),)
		}</div>`
	}
		...
		const genMarkdown = () => {
			...
			try {
				...
				turndown.addRule('dropBadges', {
					filter: (node,) =>
						node.nodeType === 1 &&
						/badge|tag|chip|pill|avatar|logo|icon|subscribe|share|comments|related/i
							.test(
								node.className || '',
							),
					replacement: () => '',
	...
	function extractMeta() {
		...
		const meta = {
			...
			subtitle: get('meta[name="description"]',) ||
		...
		try {
			...
			for (const s of scripts) {
				...
				const data = Array.isArray(json,)
					? json.find(
						(x,) => x && (x['@type'] === 'Article' || x['@type'] === 'BlogPosting'),
					)
					: json
				if (
					data &&
					...
					meta.author = typeof data.author === 'string'
						? data.author
