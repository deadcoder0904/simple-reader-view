...
(function(){
  class RuleSet {
    ...
    forNode(node) { return this.rules.filter(r => r.filter(node)); }
  }
  ...
  class TurndownService {
    ...
    nodeToMd(node) {
      ...
      switch (name) {
        case 'h1': return `# ${this.inline(el)}\n\n`;
        case 'h2': return `## ${this.inline(el)}\n\n`;
        case 'h3': return `### ${this.inline(el)}\n\n`;
        case 'h4': return `#### ${this.inline(el)}\n\n`;
        case 'h5': return `##### ${this.inline(el)}\n\n`;
        case 'h6': return `###### ${this.inline(el)}\n\n`;
        case 'p': return `${this.inline(el)}\n\n`;
        case 'br': return `\n`;
        case 'em':
        case 'i': return `*${this.inline(el)}*`;
        case 'strong':
        case 'b': return `**${this.inline(el)}**`;
        case 'code': {
          // Inline code if parent not PRE
          if (el.parentElement && el.parentElement.nodeName === 'PRE') return el.textContent || '';
        ...
        case 'ul': return this.list(el, '- ');
        case 'ol': return this.list(el, (i) => `${i + 1}. `);
        case 'li': return this.childrenToMd(el) + '\n';
        case 'blockquote': return this.blockquote(el);
        case 'hr': return `\n---\n\n`;
        default:
