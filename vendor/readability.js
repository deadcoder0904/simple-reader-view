...
;(function () {
	...
	class SimpleReadability {
		...
		parse() {
			...
			const pick = (...sels) =>
				sels.map((s,) => d.querySelector(s,)).find(Boolean,)

			// Candidate root
			let root = pick('article',) || pick('main',) ||
				pick('#content, #main, #page, .post, .article, .entry, .content',) || d.body
			...
			;(function walk(node,) {
				for (const child of Array.from(node.childNodes,)) {
					if (child.nodeType === Node.TEXT_NODE) {
						...
						if (text.trim()) {
							stack[stack.length - 1].appendChild(d.createTextNode(text,),)
						}
						continue
					}
					...
					if (
						/share|promo|advert|ad-|ads|banner|subscribe|newsletter|paywall|modal|overlay|tooltip|icon|badge|tag|chip|pill|avatar|logo|breadcrumbs|author|related|comments|popup/i
							.test(s,)
					) continue
					if (!allow) {
						walk(child,)
			...
			const title = (d.querySelector('meta[property="og:title"]',)?.content) ||
				d.title || ''
	...
	window.isProbablyReaderable = function () {
		return true
	}
})()
